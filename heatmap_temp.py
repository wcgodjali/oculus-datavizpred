...
def temp_map():
    ...
    merged.plot(column=variable, cmap='Reds', ax=ax, linewidth=0.8, edgecolor='0.8')
    
    # remove the axis
    ax.axis('off')
    
    # add a title
    ax.set_title('Real-time Average Temperature in D.I.Y. Yogyakarta', fontdict={'fontsize': '18', 'fontweight' : '3'})
    ...
    fig.savefig("map_temp.png", dpi=300)
    img=cv2.imread('map_temp.png')
    resized=cv2.resize(img,(720,480))
    cv2.imwrite("send_map_temp.jpg",resized)